...
@dp.message(MainMenuState.edit_country)
async def edit_country_back_text(message: types.Message, state: FSMContext):
    user_id = message.from_user.id
    if is_back(user_id, message.text):
        await message.answer(t(user_id, 'edit_title'), reply_markup=get_edit_profile_keyboard(user_id))
        await state.set_state(MainMenuState.editing_profile)
...
@dp.message(MainMenuState.edit_interests)
async def edit_interests_back_text(message: types.Message, state: FSMContext):
    user_id = message.from_user.id
    if is_back(user_id, message.text):
        await message.answer(t(user_id, 'edit_title'), reply_markup=get_edit_profile_keyboard(user_id))
        await state.set_state(MainMenuState.editing_profile)
