...
def get_admin_keyboard():
    builder = ReplyKeyboardBuilder()
    builder.button(text="📋 Жалобы"); builder.button(text="👥 Управление пользователями")
    builder.button(text="📊 Статистика"); builder.button(text="📢 Рассылка")
    builder.button(text="◀️ Назад"); builder.adjust(2)
    return builder.as_markup(resize_keyboard=True)
...
def get_positive_tags_keyboard(date_id, stars, selected=None):
    if selected is None: selected = []
    builder = InlineKeyboardBuilder()
    for tag in POSITIVE_TAGS:
        prefix = "✅ " if tag in selected else ""
        builder.button(text=f"{prefix}{tag}", callback_data=f"pos_tag_{date_id}_{stars}_{tag}")
    builder.button(text="✅ Готово / Done", callback_data=f"done_pos_tags_{date_id}_{stars}")
    builder.adjust(1)
    return builder.as_markup()

def get_negative_tags_keyboard(date_id, stars, selected=None):
    if selected is None: selected = []
    builder = InlineKeyboardBuilder()
    for tag in NEGATIVE_TAGS:
        prefix = "✅ " if tag in selected else ""
        builder.button(text=f"{prefix}{tag}", callback_data=f"neg_tag_{date_id}_{stars}_{tag}")
...
def get_complaint_types_keyboard(from_user_id, to_user_id):
    builder = InlineKeyboardBuilder()
    has_date = db.has_completed_date_between(from_user_id, to_user_id)
    for complaint_type in COMPLAINT_TYPES:
        if complaint_type == 'Не пришёл на встречу' and not has_date: continue
...
def extract_gender_from_text(text):
    if "Мужской" in text or "Male" in text or "მამრობითი" in text: return "M"
    elif "Женский" in text or "Female" in text or "მდედრობითი" in text: return "F"
    return None

def get_opposite_gender(gender): return "F" if gender == "M" else "M"

def is_back(user_id, text): return text in ("◀️ Назад", "◀️ Back", "◀️ უკან")

def is_bio_skip(user_id, text): return text in ("⏩ Пропустить", "⏩ Skip", "⏩ გამოტოვება")
...
def get_next_profile_to_show(user_id):
    ...
    for u in all_users:
        ...
        if user_obj['is_banned'] or user_obj['is_shadow_banned']: continue
...
def is_menu_button(user_id, text):
    menu_keys = ['menu_feed', 'menu_likes', 'menu_matches', 'menu_profile', 'menu_photos',
...
async def choose_language_reg(query: types.CallbackQuery, state: FSMContext):
    ...
    lang_map = {"lang_ru": "ru", "lang_en": "en", "lang_ka": "ka"}
    lang = lang_map.get(query.data)
    if not lang: return
    set_user_lang(user_id, lang)
    await state.update_data(language=lang)
    await query.message.answer(t(user_id, 'choose_country'), reply_markup=get_country_keyboard(user_id))
...
async def choose_country_reg(query: types.CallbackQuery, state: FSMContext):
    user_id = query.from_user.id
    if not query.data.startswith("country_"): return
    country_key = query.data.replace("country_", "")
    if country_key not in COUNTRIES: return
    await state.update_data(country=country_key)
    await query.message.answer(t(user_id, 'welcome_new'), reply_markup=get_back_keyboard(user_id), parse_mode="Markdown")
...
async def process_gender(message: types.Message, state: FSMContext):
    user_id = message.from_user.id
    if is_back(user_id, message.text):
        await message.answer(t(user_id, 'enter_name'), reply_markup=get_back_keyboard(user_id), parse_mode="Markdown")
...
async def process_city(message: types.Message, state: FSMContext):
    ...
    data = await state.get_data()
    country = data.get('country', 'belarus')
    valid_cities = COUNTRIES.get(country, {}).get('cities', ALL_CITIES)
    if city not in valid_cities: await message.answer(t(user_id, 'city_invalid')); return
...
async def process_photos(message: types.Message, state: FSMContext):
    ...
    if message.text and is_back(user_id, message.text):
        data = await state.get_data(); country = data.get('country', 'belarus')
        await message.answer(t(user_id, 'choose_city'), reply_markup=get_cities_keyboard(user_id, country))
        await state.set_state(RegistrationState.waiting_for_city); return
...
async def process_bio(message: types.Message, state: FSMContext):
    ...
    if message.text and is_bio_skip(user_id, message.text):
        ...
        await message.answer(t(user_id, 'choose_interests'), reply_markup=get_interests_keyboard(user_id))
...
async def process_interests(query: types.CallbackQuery, state: FSMContext):
    ...
    if query.data == "interests_done":
        ...
        for photo_id in user_data['photos']: db.add_photo(user_id, photo_id)
...
async def show_feed(message: types.Message, state: FSMContext):
    ...
    if user['is_banned']: await message.answer(t(user_id, 'feed_banned')); return
...
async def show_incoming_likes(message: types.Message, state: FSMContext):
    ...
    for liker_id in incoming:
        ...
        if liker and not liker['is_banned'] and not liker['is_shadow_banned']:
...
async def menu_from_feed(message: types.Message, state: FSMContext):
    ...
    elif text == t(user_id, 'menu_photos'): await view_photos(message, state)
    elif text == t(user_id, 'menu_edit'): await edit_profile(message, state)
    elif text == t(user_id, 'menu_support'): await support(message, state)
    elif text == t(user_id, 'menu_delete'): await delete_profile_prompt(message, state)
    elif text == t(user_id, 'menu_language'): await change_language(message, state)
...
async def report_profile(query: types.CallbackQuery, state: FSMContext):
    ...
    await query.message.answer(t(user_id, 'complaint_choose'), reply_markup=get_complaint_types_keyboard(user_id, to_user_id))
...
async def show_matches(message: types.Message, state: FSMContext):
    user_id = message.from_user.id; matches = db.get_user_matches(user_id)
...
async def rate_stars_selected(query: types.CallbackQuery, state: FSMContext):
    ...
    await query.message.answer(t(user_id, 'rate_positive'), reply_markup=get_positive_tags_keyboard(date_id, stars, []))
...
async def select_positive_tag(query: types.CallbackQuery, state: FSMContext):
    ...
    try: await query.message.edit_reply_markup(reply_markup=get_positive_tags_keyboard(date_id, stars, pos_tags))
    except: pass
    await query.answer()
...
async def done_positive_tags(query: types.CallbackQuery, state: FSMContext):
    ...
    await query.message.answer(t(user_id, 'rate_negative'), reply_markup=get_negative_tags_keyboard(date_id, stars, neg_tags))
...
async def select_negative_tag(query: types.CallbackQuery, state: FSMContext):
    ...
    try: await query.message.edit_reply_markup(reply_markup=get_negative_tags_keyboard(date_id, stars, neg_tags))
    except: pass
    await query.answer()
...
async def show_profile(message: types.Message, state: FSMContext):
    ...
    text += t(user_id, 'profile_age', age=user['age']) + t(user_id, 'profile_city', city=user['city'])
...
@dp.message(MainMenuState.main_menu, lambda m: m.text and any(m.text == t(m.from_user.id, k) for k in ['menu_photos']))
async def view_photos(message: types.Message, state: FSMContext):
    user_id = message.from_user.id; photos = db.get_user_photos(user_id)
...
@dp.message(MainMenuState.main_menu, lambda m: m.text and any(m.text == t(m.from_user.id, k) for k in ['menu_edit']))
...
async def edit_city_start(query: types.CallbackQuery, state: FSMContext):
    user_id = query.from_user.id
    # Determine user's country from current city
    user = db.get_user(user_id)
    user_country = None
...
async def edit_city_process(message: types.Message, state: FSMContext):
    ...
    if is_back(user_id, message.text):
        ...
        await state.set_state(MainMenuState.editing_profile); return
    city = message.text.strip()
    if city not in ALL_CITIES: await message.answer(t(user_id, 'city_invalid')); return
    db.update_user(user_id, city=city); await message.answer(t(user_id, 'edit_saved'))
    await message.answer(t(user_id, 'action_choose'), reply_markup=get_main_menu_keyboard(user_id))
    await state.set_state(MainMenuState.main_menu)
...
@dp.message(MainMenuState.main_menu, lambda m: m.text and any(m.text == t(m.from_user.id, k) for k in ['menu_language']))
async def change_language(message: types.Message, state: FSMContext):
    await message.answer("🌐 Выберите язык / Choose language / აირჩიეთ ენა:", reply_markup=get_language_keyboard())

@dp.callback_query(F.data.in_({"lang_ru", "lang_en", "lang_ka"}))
async def language_selected(query: types.CallbackQuery, state: FSMContext):
    user_id = query.from_user.id
    lang = {"lang_ru": "ru", "lang_en": "en", "lang_ka": "ka"}.get(query.data, "ru")
    set_user_lang(user_id, lang); db.update_user(user_id, language=lang)
    await query.message.answer(t(user_id, 'language_changed'))
    await query.message.answer(t(user_id, 'action_choose'), reply_markup=get_main_menu_keyboard(user_id))
    await state.set_state(MainMenuState.main_menu)

@dp.message(MainMenuState.main_menu, lambda m: m.text and any(m.text == t(m.from_user.id, k) for k in ['menu_delete']))
async def delete_profile_prompt(message: types.Message, state: FSMContext):
    user_id = message.from_user.id; builder = InlineKeyboardBuilder()
    ...
    await message.answer(t(user_id, 'delete_confirm'), reply_markup=builder.as_markup())
...
async def show_complaints(message: types.Message, state: FSMContext):
    ...
    if not complaints: await message.answer("✅ Нет новых жалоб"); return
...
async def resolve_complaint(query: types.CallbackQuery):
    ...
    if status == "approved":
        complaint = db.get_complaint(complaint_id); db.ban_user(complaint['to_user_id'], shadow_ban=True)
...
async def manage_users(message: types.Message, state: FSMContext):
    if message.from_user.id != ADMIN_ID: return
    await message.answer("Введите ID пользователя для поиска:"); await state.set_state(MainMenuState.admin_search_user)
...
async def search_user(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        builder.button(text="🔄 Обнулить анкету", callback_data=f"admin_full_reset_{user_id}"); builder.adjust(1)
        await message.answer(text, reply_markup=builder.as_markup()); await state.set_state(MainMenuState.in_admin)
    except ValueError: await message.answer("❌ Введите корректный ID")

@dp.callback_query(F.data.startswith("admin_ban_"))
async def admin_ban_user(query: types.CallbackQuery):
    if query.from_user.id != ADMIN_ID: return
    user_id = int(query.data.split("_")[2]); db.ban_user(user_id, shadow_ban=False)
...
async def admin_shadow_ban_user(query: types.CallbackQuery):
    if query.from_user.id != ADMIN_ID: return
    user_id = int(query.data.split("_")[3]); db.ban_user(user_id, shadow_ban=True)
...
async def admin_full_reset(query: types.CallbackQuery):
    ...
    if db.full_reset_user_profile(user_id): await query.answer("✅ Анкета обнулена")
    else: await query.answer("❌ Ошибка")
...
async def show_stats(message: types.Message):
    if message.from_user.id != ADMIN_ID: return
    stats = db.get_stats()
    text = f"📊 Статистика:\n\n👥 Всего пользователей: {stats['total_users']}\n❤️ Всего мэтчей: {stats['total_matches']}\n✅ Подтверждённых свиданий: {stats['confirmed_dates']}\n\n📍 По городам:\n"
...
async def broadcast(message: types.Message, state: FSMContext):
    if message.from_user.id != ADMIN_ID: return
    await message.answer("Введите текст рассылки (или ◀️ Назад для отмены):"); await state.set_state(MainMenuState.admin_broadcast)
...
async def send_broadcast(message: types.Message, state: FSMContext):
    ...
    await message.answer("Выберите действие:", reply_markup=get_admin_keyboard()); await state.set_state(MainMenuState.in_admin)
