...
def get_bio_keyboard(user_id: int) -> ReplyKeyboardMarkup:
    """Keyboard for bio step with skip option."""
    builder = ReplyKeyboardBuilder()
...
def get_country_keyboard(user_id: int) -> InlineKeyboardMarkup:
    """Country selection keyboard."""
    builder = InlineKeyboardBuilder()
    lang = get_user_lang(user_id)
    for key, val in COUNTRIES.items():
        label = val.get(lang, val['en'])
...
def get_cities_keyboard(user_id: int, country: str = None) -> ReplyKeyboardMarkup:
    builder = ReplyKeyboardBuilder()
...
def get_positive_tags_keyboard(date_id: int, stars: int, selected: list = None) -> InlineKeyboardMarkup:
    if selected is None:
        selected = []
    builder = InlineKeyboardBuilder()
...
def get_negative_tags_keyboard(date_id: int, stars: int, selected: list = None) -> InlineKeyboardMarkup:
    if selected is None:
        selected = []
    builder = InlineKeyboardBuilder()
...
def get_anonymity_keyboard(user_id: int, date_id: int, stars: int) -> InlineKeyboardMarkup:
    """Keyboard for choosing anonymous or named review."""
    builder = InlineKeyboardBuilder()
    builder.button(text=t(user_id, 'review_with_name'), callback_data=f"review_named_{date_id}_{stars}")
    builder.button(text=t(user_id, 'review_anonymous'), callback_data=f"review_anon_{date_id}_{stars}")
    builder.adjust(1)
    return builder.as_markup()
...
def extract_gender_from_text(text: str) -> Optional[str]:
    if "Мужской" in text or "Male" in text or "მამაკაცი" in text:
        return "M"
    elif "Женский" in text or "Female" in text or "ქალი" in text:
        return "F"
    return None
...
def is_back(user_id: int, text: str) -> bool:
    ...
    return text in ("◀️ Назад", "◀️ Back", "◀️ უკან")

def is_bio_skip(user_id: int, text: str) -> bool:
    """Check if the text is a bio skip button in any language."""
    return text in ("⏩ Пропустить", "⏩ Skip", "⏩ გამოტოვება")
...
def build_profile_caption(profile: dict, user_id: int) -> str:
    ...
    caption += ("отзывов" if get_user_lang(user_id) == 'ru' else ("მიმოხილვა" if get_user_lang(user_id) == 'ka' else "reviews")) + ")\n\n"
    if profile['bio']:
...
async def cmd_start(message: types.Message, state: FSMContext):
    ...
    if user and user['registration_complete']:
        ...
        await message.answer(
            "🌐 Выберите язык / Choose language / აირჩიეთ ენა:",
...
async def choose_language_reg(query: types.CallbackQuery, state: FSMContext):
    ...
    elif query.data == "lang_ka":
        set_user_lang(user_id, 'ka')
        await state.update_data(language='ka')
    else:
        return
    ...
    await query.message.answer(
        ...
        reply_markup=get_country_keyboard(user_id)
    )
...
async def process_age(message: types.Message, state: FSMContext):
    ...
    await message.answer(
        ...
        reply_markup=get_cities_keyboard(user_id, country)
    )
...
async def process_photos(message: types.Message, state: FSMContext):
    ...
    if message.photo:
        ...
        await message.answer(
            ...
            reply_markup=get_bio_keyboard(user_id)
        )
...
async def process_interests(query: types.CallbackQuery, state: FSMContext):
    ...
    if query.data == "interests_done":
        ...
        country = user_data.get('country', 'belarus')
...
async def show_reviews(query: types.CallbackQuery, state: FSMContext):
    ...
    for r in summary['ratings'][:5]:
        ...
        if is_anon:
            reviewer_display = t(user_id, 'anonymous_reviewer')
        else:
            reviewer_display = r.get('reviewer_name', '?')
        ...
        if r['negative_tags']:
            try:
                ...
                pass
        text += f"{'⭐' * r['stars']} от {reviewer_display}: {positive}{negative}\n"
...
async def show_matches(message: types.Message, state: FSMContext):
    ...
    for match in matches:
        ...
        text += ("отзывов" if get_user_lang(user_id) == 'ru' else ("მიმოხილვა" if get_user_lang(user_id) == 'ka' else "reviews")) + ")\n\n"
...
async def accept_date(query: types.CallbackQuery, state: FSMContext):
    ...
    # Get names for the confirmation messages
    accepter = db.get_user(user_id)
    accepter_name = accepter['name'] if accepter else '?'
    ...
    if date_type == 'online':
        ...
        await bot.send_message(
            proposer_id,
            t(proposer_id, 'date_confirmed_online_proposer', name=accepter_name),
        ...
        await bot.send_message(
            proposer_id,
            t(proposer_id, 'date_confirmed_proposer', name=accepter_name),
            reply_markup=get_date_arrival_keyboard(proposer_id, date_id)
        )
...
async def arrived_at_date(query: types.CallbackQuery, state: FSMContext):
    ...
    my_name = my_user['name'] if my_user else '?'
    ...
    if updated['status'] == 'completed':
        ...
        await query.message.answer(
            t(user_id, 'date_both_arrived', name=partner_name),
            reply_markup=rating_kb
        )
        await bot.send_message(
            partner_id,
            t(partner_id, 'date_both_arrived', name=my_name),
...
async def select_positive_tag(query: types.CallbackQuery, state: FSMContext):
    ...
    try:
        await query.message.edit_reply_markup(
            reply_markup=get_positive_tags_keyboard(date_id, stars, pos_tags)
        )
    except:
        pass
    await query.answer()
...
async def select_negative_tag(query: types.CallbackQuery, state: FSMContext):
    ...
    try:
        await query.message.edit_reply_markup(
            reply_markup=get_negative_tags_keyboard(date_id, stars, neg_tags)
        )
    except:
        pass
    await query.answer()
...
async def done_negative_tags(query: types.CallbackQuery, state: FSMContext):
    ...
    # Ask about anonymity before saving
    await query.message.answer(
        t(user_id, 'rate_choose_anonymity'),
        reply_markup=get_anonymity_keyboard(user_id, date_id, stars)
    )

@dp.callback_query(F.data.startswith("review_named_"))
async def review_named(query: types.CallbackQuery, state: FSMContext):
    user_id = query.from_user.id
    parts = query.data.split("_")
    date_id = int(parts[2])
    stars = int(parts[3])
    await _save_review(query, state, user_id, date_id, stars, is_anonymous=False)

@dp.callback_query(F.data.startswith("review_anon_"))
async def review_anonymous(query: types.CallbackQuery, state: FSMContext):
    user_id = query.from_user.id
    parts = query.data.split("_")
    date_id = int(parts[2])
    stars = int(parts[3])
    await _save_review(query, state, user_id, date_id, stars, is_anonymous=True)

async def _save_review(query: types.CallbackQuery, state: FSMContext, user_id: int, date_id: int, stars: int, is_anonymous: bool):
    """Save the review with all collected data."""
    data = await state.get_data()
    pos_tags = data.get('pos_tags', [])
    neg_tags = data.get('neg_tags', [])
...
async def edit_city_start(query: types.CallbackQuery, state: FSMContext):
    user_id = query.from_user.id
    # Use user's country for city list
    user = db.get_user(user_id)
    country = user.get('country', 'belarus') if user else 'belarus'
    await query.message.answer(t(user_id, 'edit_choose_city'), reply_markup=get_cities_keyboard(user_id, country))
...
async def edit_city_process(message: types.Message, state: FSMContext):
    ...
    city = message.text.strip()
    # Accept any valid city from any country
...
async def search_user(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        text += f"Страна: {user.get('country', 'belarus')}\n"
