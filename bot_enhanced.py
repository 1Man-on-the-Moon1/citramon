...
dp = Dispatcher()
psychologist_requests_by_user: dict[int, deque] = {}
...
def _call_openai_psychologist_api(user_text: str, history: Optional[list[dict]] = None) -> str:
    ...
    input_items = [
        {
            "role": "system",
            "content": (
                "Ты психолог, который специализируется на отношениях и помогает "
                "пользователям приложения знакомств. Давай мягкие, практичные и "
                "краткие советы (3-8 предложений). Не ставь медицинские диагнозы. "
                "При риске для жизни или здоровья рекомендуй обратиться за "
                "экстренной помощью."
            ),
        }
    ]
...
def _check_psychologist_rate_limit(user_id: int) -> bool:
    now_ts = datetime.now().timestamp()
    window_start = now_ts - 60
    bucket = psychologist_requests_by_user.get(user_id)
    if bucket is None:
        bucket = deque()
        psychologist_requests_by_user[user_id] = bucket
    while bucket and bucket[0] < window_start:
        bucket.popleft()
    if len(bucket) >= PSYCHOLOGIST_RATE_LIMIT_PER_MIN:
        return False
    bucket.append(now_ts)
    return True

def _resolve_welcome_lang_image_path() -> Optional[str]:
    """
    Railway может запускать код из другой cwd, поэтому пробуем несколько вариантов.
    Возвращает абсолютный путь, либо None если файла нет.
    """
    candidates = []
    if WELCOME_LANG_IMAGE_PATH:
        candidates.append(WELCOME_LANG_IMAGE_PATH)
    candidates.extend([
        os.path.join(os.getcwd(), 'assets', 'welcome_lang.png'),
        os.path.join(os.path.dirname(__file__), 'assets', 'welcome_lang.png'),
        os.path.join(os.getcwd(), 'welcome_lang.png'),
    ])
    for p in candidates:
...
async def choose_language_reg(query: types.CallbackQuery, state: FSMContext):
    ...
    welcome_image_shown = bool(data.get("welcome_image_shown"))
    ...
    builder.adjust(1)
    welcome_image_path = _resolve_welcome_lang_image_path()
    if not welcome_image_shown and welcome_image_path:
        await query.message.answer_photo(
            photo=FSInputFile(welcome_image_path),
            caption=welcome_msg,
            reply_markup=builder.as_markup(),
        )
        await state.update_data(welcome_image_shown=True)
    else:
...
async def psychologist_advice(message: types.Message, state: FSMContext):
    ...
    if user_id != ADMIN_ID:
        ...
        return
    if not OPENAI_API_KEY:
        await message.answer(t(user_id, 'psychologist_no_api_key'))
        return
    await state.update_data(psychologist_history=[])
    await message.answer(t(user_id, 'psychologist_admin_intro'), reply_markup=get_back_keyboard(user_id))
    await state.set_state(MainMenuState.psychologist_chat)

@dp.message(MainMenuState.psychologist_chat)
async def psychologist_chat_message(message: types.Message, state: FSMContext):
    user_id = message.from_user.id
    text = (message.text or "").strip()
    data = await state.get_data()
    history = data.get("psychologist_history", [])
    ...
    if is_menu_button(user_id, text):
        await state.set_state(MainMenuState.main_menu)
        if text == t(user_id, 'menu_feed'):
            await show_feed(message, state)
        elif text == t(user_id, 'menu_matches'):
        ...
        elif text == t(user_id, 'menu_my_exes'):
            await my_exes(message, state)
        elif text == t(user_id, 'menu_likes'):
            await show_incoming_likes(message, state)
        elif text == t(user_id, 'menu_psychologist'):
            await psychologist_advice(message, state)
        return

    if not text:
        await message.answer(t(user_id, 'psychologist_send_text_only'))
        return

    if not _check_psychologist_rate_limit(user_id):
        await message.answer(t(user_id, 'psychologist_rate_limit', limit=PSYCHOLOGIST_RATE_LIMIT_PER_MIN))
        return

    try:
        answer = await asyncio.to_thread(_call_openai_psychologist_api, text, history)
        history.append({"role": "user", "content": text})
        history.append({"role": "assistant", "content": answer})
        history = history[-10:]
        await state.update_data(psychologist_history=history)
        await message.answer(answer)
    except urllib.error.HTTPError as e:
        logger.error("OpenAI HTTP error: %s", e)
        await message.answer(t(user_id, 'psychologist_api_error'))
    except Exception as e:
