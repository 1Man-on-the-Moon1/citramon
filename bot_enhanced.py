...
def get_main_menu_keyboard(user_id=None, is_banned=False):
    uid = user_id or 0
    builder = ReplyKeyboardBuilder()
    if is_banned:
    ...
    builder.button(text=t(uid, 'menu_exes'))
    builder.button(text=t(uid, 'menu_support'))
...
def is_menu_button(user_id, text):
    menu_keys = ['menu_feed', 'menu_likes', 'menu_matches', 'menu_profile',
                 'menu_support', 'menu_admin', 'menu_language', 'menu_exes']
...
async def cmd_start(message: types.Message, state: FSMContext):
    ...
    if user and user['is_banned']:
        ...
        await message.answer(t(user_id, 'banned_message'), reply_markup=get_main_menu_keyboard(user_id, is_banned=True))
...
async def menu_from_feed(message: types.Message, state: FSMContext):
    ...
    elif text == t(user_id, 'menu_exes'): await exes_button(message, state)
...
@dp.message(MainMenuState.main_menu, lambda m: m.text and any(m.text == t(m.from_user.id, k) for k in ['menu_exes']))
async def exes_button(message: types.Message, state: FSMContext):
    user_id = message.from_user.id
    await message.answer(t(user_id, 'exes_coming_soon'), parse_mode="Markdown")
...
async def search_user(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        builder = InlineKeyboardBuilder()
        if user['is_banned']:
            builder.button(text=t(uid, 'admin_unblock'), callback_data=f"admin_unban_{user_id}")
        else:
            builder.button(text=t(uid, 'admin_block'), callback_data=f"admin_ban_{user_id}")
        builder.button(text=t(uid, 'admin_reset_reviews'), callback_data=f"admin_reset_reviews_{user_id}")
        builder.adjust(2)
        await message.answer(text, reply_markup=builder.as_markup(), parse_mode="HTML")
        await state.set_state(MainMenuState.in_admin)
...
async def admin_ban_user(query: types.CallbackQuery):
    ...
    await query.answer(t(uid, 'admin_block_success'))
...
async def admin_unban_user(query: types.CallbackQuery):
    ...
    await query.answer(t(uid, 'admin_unblock_success'))

@dp.callback_query(F.data.startswith("admin_reset_reviews_"))
async def admin_reset_reviews(query: types.CallbackQuery):
    if query.from_user.id != ADMIN_ID: return
    uid = query.from_user.id
    user_id = int(query.data.split("_")[3]); db.reset_user_reviews(user_id)
    await query.answer(t(uid, 'admin_reset_reviews_success'))
