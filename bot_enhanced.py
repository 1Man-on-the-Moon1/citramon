...
def get_admin_keyboard():
    ...
    builder.button(text="👋 Приветствие"); builder.button(text="◀️ Назад")
    builder.adjust(2)
    return builder.as_markup(resize_keyboard=True)
...
def is_menu_button(user_id, text):
    menu_keys = ['menu_feed', 'menu_likes', 'menu_matches', 'menu_profile',
                 'menu_edit', 'menu_support', 'menu_admin', 'menu_language']
...
async def choose_country_reg(query: types.CallbackQuery, state: FSMContext):
    ...
    # Go to city selection right after country
    await query.message.answer(t(user_id, 'choose_city'), reply_markup=get_cities_keyboard(user_id, country_key))
...
async def process_city(message: types.Message, state: FSMContext):
    ...
    if is_back(user_id, message.text):
        ...
        await state.set_state(RegistrationState.choosing_country); return
    ...
    # Go to name input after city
    await message.answer(t(user_id, 'welcome_new'), reply_markup=get_back_keyboard(user_id), parse_mode="Markdown")
...
@dp.message(MainMenuState.in_admin, F.text == "👋 Приветствие")
