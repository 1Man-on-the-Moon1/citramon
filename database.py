...
def _ensure_db_path(path: str) -> str:
    """Ensure database directory/file path is writable before sqlite connect."""
...
def _resolve_database_path() -> str:
    ...
    candidates = [
        env_path,
        "/data/vibestar.db",        # Railway volume mount
        "/app/data/vibestar.db",     # Alternative volume mount
        os.path.join(os.getcwd(), "vibestar.db"),
        "/tmp/vibestar.db",          # Always writable fallback
    ]
...
def init_db():
    ...
    # Add country column if missing (migration for existing DBs)
    try:
        cursor.execute("ALTER TABLE users ADD COLUMN country TEXT DEFAULT 'belarus'")
    except sqlite3.OperationalError:
        pass
    ...
    # Add is_anonymous column to ratings if missing (migration)
    try:
        cursor.execute("ALTER TABLE ratings ADD COLUMN is_anonymous BOOLEAN DEFAULT 0")
    except sqlite3.OperationalError:
        pass
...
class Database:
    ...
    # User operations
    def create_user(self, user_id: int, name: str, gender: str, age: int, city: str, language: str = 'ru', country: str = 'belarus') -> bool:
        ...
        try:
            ...
            cursor.execute('''
                INSERT INTO users (user_id, name, gender, age, city, rating, language, country)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?)
            ''', (user_id, name, gender, age, city, rating, language, country))
            conn.commit()
            return True
    ...
    def add_rating(self, date_id: int, from_user_id: int, to_user_id: int, 
                   stars: int, positive_tags: List[str] = None, negative_tags: List[str] = None,
        ...
        try:
            ...
            cursor.execute('''
                INSERT INTO ratings (date_id, from_user_id, to_user_id, stars, positive_tags, negative_tags, is_anonymous, is_public, public_at)
    ...
    def is_dating_confirmed_by_both(self, match_id: int) -> bool:
        ...
        conn.close()
        if row:
            return bool(row['user1_confirmed'] and row['user2_confirmed'])
        return False
