...
class Database:
    ...
    def create_user(self, user_id, name, gender, age, city, language='ru'):
        ...
        try:
            ...
            cursor.execute('INSERT INTO users (user_id, name, gender, age, city, rating, language) VALUES (?, ?, ?, ?, ?, ?, ?)',
    ...
    def ban_user(self, user_id, shadow_ban=False):
        conn = self.get_connection(); cursor = conn.cursor()
        try:
            if shadow_ban: cursor.execute('UPDATE users SET is_shadow_banned = 1 WHERE user_id = ?', (user_id,))
            else: cursor.execute('UPDATE users SET is_banned = 1 WHERE user_id = ?', (user_id,))
            conn.commit(); return True
        finally: conn.close()
    def unban_user(self, user_id):
        conn = self.get_connection(); cursor = conn.cursor()
        try:
            cursor.execute('UPDATE users SET is_banned = 0, is_shadow_banned = 0 WHERE user_id = ?', (user_id,))
            conn.commit(); return True
        finally: conn.close()
